...
//sequential search
int sequential_search(int data[], int banyakdata, int angka)
{
    int i ;
	for(i = 0; i < banyakdata ; i++)
	{
...
//binary search
int binary_search(int data[],int min,int max, int angka)
{
	int mid ;
    if (max >= min)
	{
		...
		else if (data[mid] > angka)
		{
            return binary_search(data, min, mid-1, angka) ;	
		}
		else
		{
			return binary_search(data, mid+1, max, angka) ;
		}
   }
   return -1 ;
}
...
float durasi_quick_sort(int data[], int banyakdata)
{
    ...
    clock_t end = clock() ;
    float wkt = (float)(end - start) / CLOCKS_PER_SEC ;
    printf("\n Durasi waktu yang di perlukan untuk sorting dengan menggunakan metode Quick Sort = %lf s", wkt) ;
}
...
float durasi_binary_search(int data[], int banyakdata, int bilangan)
{
    ...
    clock_t end = clock() ;
    float wkt = (float)(end - start) / CLOCKS_PER_SEC ;
    printf("\n Durasi waktu yang di perlukan untuk searching dengan menggunakan metode Binary Search = %lf", wkt) ;
    if(array == -1)
	{
        printf("\n Bilangan %d tidak terdapat pada array\n", bilangan) ;  
    }
