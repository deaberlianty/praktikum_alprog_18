...
int main(){
	...
	do{
		...
		if(((scanf(" %d%c", &pilihan, &validasi)) != 2 || validasi != '\n') && clean()){ 
			...
			printf("\tInputan salah! hanya dapat memilih menu 1-3.\n\n"); 
			}
		}
	}while(1);
...
void hanoiRekursif(int jmlhCakram, int asal, int tujuan, int temp){
	if (jmlhCakram > 0) { 
        hanoiRekursif(jmlhCakram-1, asal, temp, tujuan);//pindah tower 1 ke 2
        printf("\t Memindahkan Cakram paling atas dari Tower %d ke Tower %d\n", asal, tujuan); // 1 ke 3
        hanoiRekursif(jmlhCakram-1, temp, tujuan, asal); // 2 ke 3
    }
}
...
void towerHanoiIteratif(){
	...
	do{
		printf("\n\t Masukkan jumlah cakram: ");
		if(((scanf("%d%c", &cakram, &validasi)) != 2 || validasi != '\n') && clean()){ 
			printf("\t Inputan salah! hanya dapat menginputkan angka\n\n"); 
		}
		else{
			if(cakram <= 0){ 
				printf("\t Inputan salah! Mohon hanya memasukkan angka lebih dari 0\n\n"); 
	...
	for(i=1; i < (1<<cakram); i++){  //bitwise left shift nilai dari 1 akan digeser kekiri sebaanyak jumlah cakram
		counter++; 
